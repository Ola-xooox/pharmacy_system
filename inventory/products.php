<?php
    ...
    // Fetch only non-expired products for the main product view
    $products_result = $conn->query("SELECT p.*, c.name as category_name FROM products p JOIN categories c ON p.category_id = c.id WHERE (p.expiration_date > CURDATE() OR p.expiration_date IS NULL) ORDER BY p.id DESC");
    $products = [];
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-6">
                ...
                <div class="mb-6 relative">
                    <input type="text" id="product-search-input" placeholder="Search by name, category, or lot number..." class="w-full pl-10 pr-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div class="sm:col-span-2 grid grid-cols-3 gap-4">
                                ...
                                <input type="text" name="item_total" placeholder="Total Value to Add" class="form-input">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
                ...
                try {
                    ...
                    if (result.success) {
                        if (result.action === 'updated') {
                            const index = products.findIndex(p => p.id == result.product.id);
                            if (index !== -1) {
                ...
                return `
                    <div class="product-card" data-product-id="${product.id}">
                        ...
                        <div class="product-info text-center">
                            ...
                            <p class="text-xs text-gray-400 mb-2">Stock: ${product.stock} | Total: ${Number(product.item_total)}</p>
            ...
            function createCategoryButtonHTML(category, isActive = false) {
                 return `<button class="category-btn ${isActive ? 'active' : ''}" data-id="${category.id}">${category.name}</button>`;
            }
            function renderCategories() {
                let buttonsHTML = createCategoryButtonHTML({ id: 'all', name: 'All Products' }, true);
            ...
            function updateProductView() {
                ...
                const activeCategoryId = activeCategoryBtn ? activeCategoryBtn.dataset.id : 'all';
                const searchTerm = searchInput.value.toLowerCase();
                let filteredProducts = products;
                if (activeCategoryId !== 'all') {
                    filteredProducts = filteredProducts.filter(product => product.category_id == activeCategoryId);
                }
                if (searchTerm) {
                    filteredProducts = filteredProducts.filter(product => 
                        product.name.toLowerCase().includes(searchTerm) ||
                        product.category_name.toLowerCase().includes(searchTerm) ||
                        (product.lot_number && product.lot_number.toLowerCase().includes(searchTerm))
                    );
                }
